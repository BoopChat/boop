...
const TwitterButton = ({ text="sign up with Twitter" }) => {
    //send action to redux store to change states
    const dispatch = useDispatch();

    //Handler function for Twitter login button
    const handleLogin = async () => {
        ...
        if (success) {
            ...
            window.location.replace("/api/login/auth/twitter", "_self");
        }
    };

    return (
        <button className="twitter" type="button" onClick={handleLogin}>
